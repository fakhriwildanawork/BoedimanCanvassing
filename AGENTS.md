...
* Setiap kali ada perubahan struktur database table, Anda **WAJIB** memperbarui file schema SQL yang relevan di folder `/database/` (misal: `/database/schema.sql`) agar selalu sinkron dan akurat dengan schema TypeScript/database aslinya.
* API Keys dan rahasia lainnya **MUST** diakses via `process.env` or `import.meta.env`. Jangan pernah melakukan *hardcode* kredensial.